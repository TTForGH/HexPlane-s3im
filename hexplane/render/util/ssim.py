...
def create_window(window_size, channel):
    ...
    window = Variable(_2D_window.expand(channel, 1, window_size, window_size).contiguous()) # contiguous（） 切断函数之间联系
    return window
...
def _ssim(img1, img2, window, window_size, channel, size_average = True, stride=None):
    ...
    if size_average:
        ...
        '''
        [batch_size, channels, height, width], mean(1) -> [batch_size, height, width], mean(1)
         -> [batch_size, width], mean(1) -> [batch_size] 包含每个图像的平均SSIM值
        '''
